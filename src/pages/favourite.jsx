...
export default function Home() {
  ...
  const handleDeletePoster = async (imdbID) => {
    try {
      const response = await fetch(`http://localhost:8000/posters/${imdbID}`, {
        method: 'DELETE',
      });
          ...
          {searchResults.map((result,index) => (
            <li key={index} className='mx-3'>
              ...
              <Link href={`/singleMovie/${result.imdbID}`}>
                ...
                <button className='bg-red-700 text-white rounded px-4 h-[40px] text-[20px] mt-4' onClick={()=>handleDeletePoster(result.imdbID)} >Delete</button>
              </div>
            </li>
